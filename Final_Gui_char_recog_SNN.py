...
class DigitRecognizerApp:
    ...
    #     img_cropped = img[y_min:y_max+1, x_min:x_max+1]
        ...
        aspect_ratio = w / h
        new_size = 28 - 2 * margin  # Target size
    ...
    def predict(self):
        ...
        with torch.no_grad():
            ...
            global outtensor_prob, start_anim, predictions, output_histogram, hidden_spikes

            hidden_spikes = hook_output
            outtensor_prob = torch.softmax(0.01*outtensor,1)
            output_activations = outtensor_prob.cpu().numpy()
            predictions = np.argmax(outtensor, axis=1)
...
# Live prediction (most active neuron)
    ...
    def clear_canvas(self):
        ...
        self.image = Image.new("L", (GUI_Width, GUI_Height), "black")
        self.draw = ImageDraw.Draw(self.image)
        global start_anim, frame
        start_anim = False
        frame = 0
#%%
fig, ax = plt.subplots(figsize=(8, 6))
fig.suptitle("Neural Network Simulation", fontsize=14)
...
output_nodes = ax.scatter(output_positions, [honodes[1]] * num_output, s=200, color="red", label="Output Neurons")

for ctr in output_positions:
    i = ctr
    j = honodes[1]
    corr = 0
    ax.annotate(f'{int(ctr)}', xy=(i+corr,j+corr), fontsize=12, ha='center', va='center')
    #ctr+=1
...
for i,bar in enumerate(bars):
    labels.append(ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.05, 
                  f"{i}", ha='center', va='bottom', fontsize=10) )
...
def update(_):
    ...
    global frame
    if start_anim and frame < T*intperspike:
        curr_tim = int(np.floor(frame/intperspike))
        # Update histogram bars
        for i, bar in enumerate(bars):
            bar.set_height(output_histogram[curr_tim][i])
            labels[i].set_position((bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.05))
            labels[i].set_text(f"{i}")  # Update label value
...
ani = animation.FuncAnimation(fig, update, frames=intperspike*(time_steps*150), interval=100, blit=False, repeat=False)
